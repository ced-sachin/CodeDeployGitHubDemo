...
if ( ! class_exists( 'Class_Ced_Onbuy_Products' ) ) {
	...
	class Class_Ced_Onbuy_Products {
		...
		public function ced_doUpdateInventory( $shop_id ) {
			// echo '<pre>'; print_r($this->data); die(__METHOD__);
			$response = $this->update_stock_to_onbuy( $this->data, $shop_id );
			// echo '<pre>'; print_r($response); die(__METHOD__);
			return $response;
		}
		...
		public function update_stock_to_onbuy( $parameters, $shop_id ) {
			...
			// echo '<pre>'; print_r($parameters); die(__METHOD__);	
				...
				if ( isset( $response['results'] ) && ! empty( $response['results'] ) && is_array( $response['results'] ) ) {
					foreach ( $prepared_data as $key1 => $value1 ) {
						foreach ( $response['results'] as $key => $value ) {
							...
							if ( $key1 == $key ) {
								// ========= FOR CRON ACTIVITY ==================
								global $onbuy_activity;
								$onbuy_activity->action        = 'Upload';
								$onbuy_activity->type          = 'product';
								$onbuy_activity->input_payload = $value1;
								$onbuy_activity->response      = $value;
								$onbuy_activity->post_id       = $value1['uid'];
								$onbuy_activity->shop_id       = $shop_id;
								$onbuy_activity->is_auto       = $is_sync;
								$onbuy_activity->post_title    = $value1['product_name'];
								$onbuy_activity->execute();
								// ===================================
							}
						}
					}
		...
		public function ced_onbuy_prepare_data_for_updating( $pro_ids = array(), $shop_id, $is_sync ) {
			...
			foreach ( $pro_ids as $key => $value ) {
				...
				if ( isset( $onbuy_item_id ) && ! empty( $onbuy_item_id ) ) {
					...
					if ( 'variable' == $type ) {
						...
						echo '<pre>'; print_r($prepared_data); die(__METHOD__);
					}
				} else {
			...
			// ========= FOR CRON ACTIVITY ==================
			global $onbuy_activity;
			$onbuy_activity->action        = 'Update';
			$onbuy_activity->type          = 'product';
			$onbuy_activity->input_payload = $prepared_data;
			$onbuy_activity->response      = $response;
			$onbuy_activity->post_id       = $this->prod_id;
			$onbuy_activity->shop_id       = $shop_id;
			$onbuy_activity->is_auto       = $is_sync;
			$onbuy_activity->post_title    = $prod_data->get_title();
			$onbuy_activity->execute();
			// ===================================
			return $response_data;
		}
		...
		public function ced_onbuy_prepare_data_for_delete( $pro_ids = array(), $shop_id, $is_sync ) {
			...
			// ========= FOR CRON ACTIVITY ==================
			global $onbuy_activity;
			$onbuy_activity->action        = 'Delete';
			$onbuy_activity->type          = 'product';
			$onbuy_activity->input_payload = $skudata;
			$onbuy_activity->response      = $response;
			$onbuy_activity->post_id       = $this->prod_id;
			$onbuy_activity->shop_id       = $shop_id;
			$onbuy_activity->is_auto       = $is_sync;
		...
		public function ced_get_formatted_data_create_listing( $pro_ids = '', $shop_id = '', $onbuy_item_id = '' ) {
			...
			$handling_time           = (int) $this->ced_fetch_meta_value_of_product( $pro_ids, '_ced_onbuy_handling_time' );
			$args['delivery_weight'] = (float) get_post_meta( $pro_ids, '_weight', true );
			...
			// ====================================
			$args['handling_time'] = (int) $handling_time;
			$args['sku']           = $sku;
			return $args;
		...
		public function ced_onbuy_prepare_data_for_create_listing( $pro_ids = array(), $shop_id, $is_sync ) {
			...
			if ( isset( $pro_ids ) && ! empty( $pro_ids ) && is_array( $pro_ids ) ) {
				...
				foreach ( $pro_ids as $key => $pro_id ) {
					$profile_data        = $this->ced_onbuy_get_setting_data( $shop_id );
					$is_profile_assigned = $this->is_profile_assigned( $pro_id );
				...
				// ========= FOR CRON ACTIVITY ==================
				global $onbuy_activity;
				$onbuy_activity->action        = 'Create Listing';
				$onbuy_activity->type          = 'product';
				$onbuy_activity->input_payload = $prepared_data;
				$onbuy_activity->response      = $response;
				$onbuy_activity->post_id       = $this->prod_id;
				$onbuy_activity->shop_id       = $shop_id;
				$onbuy_activity->is_auto       = $is_sync;
				$onbuy_activity->post_title    = $product->get_title();
				$onbuy_activity->execute();
				// ===================================
				return $response_data;
			}
		...
		public function ced_onbuy_prepare_data_for_update_stock( $pro_ids = array(), $shop_id, $is_sync ) {
			...
			// var_dump($prepared_data);die;
			if ( isset( $prepared_data ) && ! empty( $prepared_data ) && is_array( $prepared_data ) ) {
				$dataToPost['site_id']  = '2000';
				$dataToPost['listings'] = $prepared_data;
				$this->data             = json_encode( $dataToPost );
				// echo '<pre>'; print_r($dataToPost); die(__METHOD__);
					...
					foreach ( $prepared_data as $key1 => $value1 ) {
						foreach ( $response['results'] as $key => $value ) {
							...
							if ( $key1 == $key ) {
								...
								global $onbuy_activity;
								$onbuy_activity->action        = 'Update';
								$onbuy_activity->type          = 'product_inventory';
								$onbuy_activity->input_payload = $value1;
								$onbuy_activity->response      = $value;
								$onbuy_activity->post_id       = $prod_id;
								$onbuy_activity->shop_id       = $shop_id;
								$onbuy_activity->is_auto       = $is_sync;
								$onbuy_activity->post_title    = $prod_data->get_title();
								$onbuy_activity->execute();
		...
		public function ced_get_formatted_data( $pro_ids = '', $shop_id = '', $onbuy_item_id = '' ) {
			...
			$handling_time = (int) get_post_meta( $pro_ids, 'ced_onbuy_handling_time', true );
			...
			// ===============================================================================================================
			// ================= CHECK IF CATEGORY EXEMPTED OR NOT ===================================
			$onbuy_exempted_list = CED_ONBUY_DIRPATH . 'admin/onbuy/json/';
			$onbuy_exempted_list = $onbuy_exempted_list . 'onbuy_exempted_categories.json';
			if ( file_exists( $onbuy_exempted_list ) ) {
				$onbuy_exempted_list = file_get_contents( $onbuy_exempted_list );
				$onbuy_categories    = json_decode( $onbuy_exempted_list, true );
			}
			foreach ( $onbuy_categories as $key => $value ) {
				if ( ! empty( $value['ID'] ) && $category_id == $value['ID'] ) {
					$product_code = '';
				}
			}
			// =======================================================================================
			...
			if ( isset( $deliveries ) && ! empty( $deliveries ) ) {
				...
				$args['listings'][ $condition ]['handling_time'] = (int) $handling_time;
			}
			$args['listings'][ $condition ]['sku']                  = $sku;
			$args['listings'][ $condition ]['delivery_weight']      = (float) get_post_meta( $pro_ids, '_weight', true );
		...
		public function ced_get_variable_formatted_data( $pro_ids = '', $shop_id = '', $onbuy_item_id = '' ) {
			...
			if ( isset( $variations ) && ! empty( $variations ) && is_array( $variations ) ) {
				foreach ( $variations as $variation ) {
					...
					$handling_time = (int) get_post_meta( $variation_id, 'ced_onbuy_handling_time', true );
					if ( empty( $handling_time ) && '' == $handling_time ) {

						$handling_time = (int) $this->ced_fetch_meta_value_of_product( $variation_id, '_ced_onbuy_handling_time' );
					...
					if ( isset( $deliveries ) && ! empty( $deliveries ) ) {
						...
						$variation_data['listings'][ $condition ]['handling_time'] = (int) $handling_time;
					}
					...
					$variation_data['listings'][ $condition ]['delivery_weight'] = (float) get_post_meta( $variation_id, '_weight', true );
		...
		public function ced_get_variable_formatted_data_inventory( $pro_ids = '', $shop_id = '', $onbuy_item_id = '' ) {
			...
			$paren_data['handling_time'] = (int) $handling_time;
			$paren_data['sku']           = $sku;
			$args[]                      = $paren_data;
			$variations                  = $product->get_available_variations();
			if ( isset( $variations ) && ! empty( $variations ) && is_array( $variations ) ) {
				foreach ( $variations as $variation ) {
					...
					if ( empty( $price ) ) {
						$price = get_post_meta( $variation_id, 'ced_onbuy_price', true );
					}
					// var_dump($_product);
					if ( empty( $price ) ) {
						$price = (float) $product_data['price'];
						if ( 'variable' == $product_type ) {
							...
							if ( isset( $variation['display_regular_price'] ) ) {
								$price = (float) $variation['display_regular_price'];
							}
						}
					}
					$stock = (int) $this->ced_fetch_meta_value_of_product( $variation_id, '_ced_onbuy_stock' );
					if ( empty( $stock ) ) {
						$stock = get_post_meta( $variation_id, 'ced_onbuy_stock', true );
					}
					if ( empty( $stock ) && '' == $stock ) {
						$stock = (int) $quantity;
					}
					...
					$variation_data['boost_marketing_commission'] = $boost_percent;
					// ====================================
					$variation_data['handling_time'] = (int) $handling_time;
