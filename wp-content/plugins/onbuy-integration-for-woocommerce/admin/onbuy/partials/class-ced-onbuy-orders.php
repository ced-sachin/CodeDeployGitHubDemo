...
class Class_CedOnbuyOrders {
	...
	public function ced_onbuy_get_the_orders( $shop_id, $is_sync = false ) {
		...
		// echo '<pre>'; print_r($orders); echo '<br>ordie dta';
		if ( isset( $orders['results'] ) ) {
			$orders = $orders['results'];
			// echo '<pre>'; print_r($orders); die(__FILE__);
	...
	public function ced_create_local_order( $orders, $shop_id = '' ) {
		// echo '<pre>'; print_r($orders); die(__METHOD__);
	...
	public function ced_create_order( $address = array(), $OrderItemsInfo = array(), $frameworkName = 'ManoMano', $orderMeta = array(), $shop_id ) {
		...
		if ( count( $OrderItemsInfo ) ) {
			...
			$onbuy_activity->action        = 'Fetch';
			$onbuy_activity->type          = 'order';
			$onbuy_activity->input_payload = $OrderItemsInfo;
			$onbuy_activity->post_title    = 'OnBuy order : ' . $OrderNumber;
			$onbuy_activity->post_id       = $OrderNumber;
			$onbuy_activity->shop_id       = $shop_id;
			$onbuy_activity->is_auto       = $this->is_sync;
			...
			$onbuy_activity->response = $final_response;
